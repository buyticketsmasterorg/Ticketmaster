...
// — AUTO-CLOSE CHAT IF USER DELETED —
useEffect(() => {
...
```
    ...
    <div className={`w-full md:w-1/3 flex flex-col h-full border-r ${darkMode ? 'border-gray-800' : 'border-gray-200'} ${selectedUser ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className={`p-4 border-b flex justify-between items-center ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
            <div className="flex items-center gap-3">
                <img src="https://upload.wikimedia.org/wikipedia/commons/6/6c/Facebook_Messenger_logo_2018.svg" className="w-8 h-8" alt="Messenger" />
    ...
    {/* --- CHAT WINDOW (Shows when user selected) --- */}
    {selectedUser && (
        <div className={`w-full md:w-2/3 h-full flex flex-col ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
            
            {/* Chat Header with BACK BUTTON */}
            <div className={`p-3 border-b flex justify-between items-center shadow-sm ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
                <div className="flex items-center gap-3">
                    {/* BACK BUTTON - Works on all browsers */}
                    <button 
