...
export default function SeatMap({ event, globalPrice, cart, setCart, onCheckout }) {
  ...
  // Panic State: 'idle', 'all-grey', 'partial-grey'
  const [panicState, setPanicState] = useState('idle');
  const [failCount, setFailCount] = useState(0);
  const [flashMsg, setFlashMsg] = useState('');

  // Define Blocks for the Arena Layout (Matches your Reference Image Style)
  const sections = [
    // FLOOR
  ...
  return (
    <div className="animate-fadeIn pb-32">
      ...
      {/* --- ARENA BLOCK OVERVIEW (LEVEL 1) --- */}
      {view === 'stadium' && (
        <div className="max-w-4xl mx-auto space-y-4 py-10">
            
            {/* STAGE */}
            <div className="w-full h-24 bg-black border-b-4 border-[#ea0042] rounded-b-3xl shadow-[0_10px_50px_rgba(234,0,66,0.1)] flex items-center justify-center mb-12">
                <span className="text-gray-600 font-black uppercase tracking-[0.5em] text-xs">Main Stage</span>
            </div>

            <div className="flex justify-center gap-4 lg:gap-8">
                {/* LEFT WING */}
                <div className="flex flex-col gap-2 w-20 lg:w-32 pt-10">
                    {sections.filter(s=>s.type==='side-left').map(s => (
                        <button key={s.id} onClick={()=>{setView('section'); setSelectedSection(s);}} className={`${s.color} h-24 rounded-lg border-2 border-white/5 hover:border-white hover:scale-105 transition-all shadow-lg flex items-center justify-center group`}><span className="text-[10px] font-bold text-gray-400 -rotate-90 group-hover:text-white">{s.name}</span></button>
                    ))}
                </div>

                {/* FLOOR (CENTER) */}
                <div className="flex-1 max-w-sm flex flex-col gap-2">
                    {sections.filter(s=>s.type==='floor').map(s => (
                        <button key={s.id} onClick={()=>{setView('section'); setSelectedSection(s);}} className={`${s.color} h-32 rounded-lg border-2 border-white/5 hover:border-white hover:scale-105 transition-all shadow-lg flex items-center justify-center group`}><span className="text-xs font-black text-blue-200 uppercase tracking-widest group-hover:text-white">{s.name}</span></button>
                    ))}
                    
                    {/* BACK SECTION */}
                    <div className="flex gap-2 mt-4">
                        {sections.filter(s=>s.type==='back').map(s => (
                            <button key={s.id} onClick={()=>{setView('section'); setSelectedSection(s);}} className={`${s.color} h-16 flex-1 rounded-lg border-2 border-white/5 hover:border-white hover:scale-105 transition-all shadow-lg flex items-center justify-center group`}><span className="text-[10px] font-bold text-gray-400 group-hover:text-white">{s.name}</span></button>
                        ))}
                    </div>
                </div>
                ...
                {/* RIGHT WING */}
                <div className="flex flex-col gap-2 w-20 lg:w-32 pt-10">
                    {sections.filter(s=>s.type==='side-right').map(s => (
                        <button key={s.id} onClick={()=>{setView('section'); setSelectedSection(s);}} className={`${s.color} h-24 rounded-lg border-2 border-white/5 hover:border-white hover:scale-105 transition-all shadow-lg flex items-center justify-center group`}><span className="text-[10px] font-bold text-gray-400 rotate-90 group-hover:text-white">{s.name}</span></button>
                    ))}
                </div>
            </div>
        ...
        <div className="animate-slideUp relative">
           ...
           <div className="bg-white text-gray-900 rounded-[40px] p-6 lg:p-10 shadow-2xl relative overflow-hidden min-h-[500px]">
              ...
              <div className="flex justify-between items-end mb-8 border-b border-gray-100 pb-4 relative z-10">
                 <div>
                    <h3 className={`text-3xl font-black italic uppercase tracking-tighter ${selectedSection.isVip ? 'text-pink-600' : 'text-gray-900'}`}>{selectedSection.name}</h3>
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{selectedSection.isVip ? 'VIP Package Includes Early Entry' : 'Standard Admission'}</p>
                 </div>
                              ...
                              ${isVisualGrey
                                ...
                                : isSelected 
                                   ...
                                   : selectedSection.isVip 
                                      ? 'bg-white border-pink-500 text-pink-600 hover:bg-pink-500 hover:text-white shadow-[0_0_10px_rgba(236,72,153,0.3)]' 
      ...
      {cart.length > 0 && (
         <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-6 z-50 animate-slideUp shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
               ...
               <button onClick={onCheckout} className="bg-[#026cdf] text-white px-8 lg:px-12 py-4 rounded-full font-black uppercase italic tracking-widest shadow-[0_10px_30px_rgba(2,108,223,0.4)] hover:scale-105 active:scale-95 transition-all">
                  Proceed to Pay
               </button>
            </div>
         </div>
