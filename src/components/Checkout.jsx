...
import { ChevronLeft, CreditCard, Lock, Calendar, ShieldCheck, Ticket, Wallet, Gift } from 'lucide-react';

export default function Checkout({ cart, onBack, onSuccess }) {
    const [paymentMethod, setPaymentMethod] = useState('card'); // card, paypal, gift
            ...
            <div className="w-full max-w-lg bg-[#1f262d] rounded-[40px] p-8 lg:p-10 border border-white/10 shadow-2xl relative overflow-hidden">
                ...
                <div className="space-y-4 relative z-10">
                    {paymentMethod === 'card' && (
                        <>
                            <div className="space-y-2">
                                ...
                                <div className="bg-[#0f131a] p-4 rounded-2xl border border-white/10 flex items-center gap-4 group focus-within:border-[#026cdf] transition-colors shadow-inner"><CreditCard className="text-gray-500 group-focus-within:text-[#026cdf]" /><input className="bg-transparent font-bold text-lg outline-none w-full placeholder:text-gray-700" placeholder="0000 0000 0000 0000" value={card} onChange={e=>setCard(e.target.value)} maxLength={19} /></div>
                            </div>
                            <div className="flex gap-4">
                                <div className="space-y-2 flex-1">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-4">Expiry</label>
                                    <div className="bg-[#0f131a] p-4 rounded-2xl border border-white/10 flex items-center gap-3 group focus-within:border-[#026cdf] transition-colors shadow-inner"><Calendar className="w-5 h-5 text-gray-500 group-focus-within:text-[#026cdf]" /><input className="bg-transparent font-bold text-lg outline-none w-full placeholder:text-gray-700" placeholder="MM/YY" value={date} onChange={e=>setDate(e.target.value)} maxLength={5} /></div>
                                </div>
                                <div className="space-y-2 flex-1">
                    ...
                    {paymentMethod === 'paypal' && (
                        <div className="bg-[#0f131a] p-8 rounded-2xl border border-white/10 text-center space-y-4">
                            <Wallet className="w-12 h-12 text-[#003087] mx-auto" />
                            <p className="text-sm font-bold text-gray-400">You will be redirected to PayPal to complete your purchase securely.</p>
                        </div>
                    )}

                    {paymentMethod === 'gift' && (
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-4">Gift Card Code</label>
                            <div className="bg-[#0f131a] p-4 rounded-2xl border border-white/10 flex items-center gap-4 group focus-within:border-[#ea0042] transition-colors shadow-inner"><Gift className="text-gray-500 group-focus-within:text-[#ea0042]" /><input className="bg-transparent font-bold text-lg outline-none w-full placeholder:text-gray-700" placeholder="XXXX-XXXX-XXXX" /></div>
                        </div>
                    )}
                ...
                <div className="mt-8 flex justify-center gap-4 opacity-20"><div className="h-8 w-12 bg-white rounded flex items-center justify-center text-[8px] font-black text-black">VISA</div><div className="h-8 w-12 bg-white rounded flex items-center justify-center text-[8px] font-black text-black">MC</div><div className="h-8 w-12 bg-white rounded flex items-center justify-center text-[8px] font-black text-black">AMEX</div></div>
            </div>
        </div>
    )
}
